...
<div class="p-6 bg-white rounded-2xl shadow">
        ...
        <table class="w-full border border-green-200 rounded-lg text-sm">
            ...
            <tbody>
                ...
                <tr class="hover:bg-green-50 transition">
                    <td class="p-3 border-b">{{ $k->kode }}</td>
                    <td class="p-3 border-b">{{ $k->nama_kriteria }}</td>
                    <td class="p-3 border-b text-center font-medium">{{ number_format($k->bobot, 2) }}</td>
                    <td class="p-3 border-b text-center">{{ ucfirst($k->tipe) }}</td>
                    <td class="p-3 border-b">
                        <ul class="list-disc list-inside text-gray-700">
                            @foreach($k->subkriteria as $sub)
                                <li>{{ $sub->nama_subkriteria }} (Nilai: {{ $sub->nilai }})</li>
                            @endforeach
                        </ul>
                    </td>
                    ...
                    <td class="p-3 border-b text-center">
                        <div class="flex justify-center gap-2">
                            ...
                            <button type="button"
                                    ...
                                    data-id="{{ $k->id }}" data-nama="{{ $k->nama_kriteria }}">
    ...
    <div class="mt-4 flex justify-between items-center">
        <span class="font-semibold text-gray-700">Total Bobot: {{ number_format($totalBobot, 2) }}</span>
        @if($totalBobot == 1)
